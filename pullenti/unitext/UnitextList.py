...
class UnitextList(UnitextItem):
            ...
            if (self.__is_all_list_items_single()): 
                ...
                while li is not None: 
                    ...
                    if (it.prefix is not None and it.content is not None): 
                        ...
                        ccc.children.append(UnitextPlaintext._new570(" ", it.prefix.end_char, it.prefix.end_char))
        ...
        if (xml0_.attrib is not None): 
            for a in xml0_.attrib.items(): 
                if (Utils.getXmlAttrLocalName(a) == "pref"): 
                    ...
                    n = 0
                    wrapn571 = RefOutArgWrapper(0)
                    inoutres572 = Utils.tryParseInt(a[1], wrapn571)
                    n = wrapn571.value
                    if (inoutres572): 
                        self.level = n
        for x in xml0_: 
    ...
    @staticmethod
    def _new355(_arg1 : int, _arg2 : object) -> 'UnitextList':
        res = UnitextList()
        res.level = _arg1
        res.tag = _arg2
        return res
    ...
    @staticmethod
    def _new356(_arg1 : int, _arg2 : object, _arg3 : str) -> 'UnitextList':
        res = UnitextList()
        res.level = _arg1
        res.tag = _arg2
