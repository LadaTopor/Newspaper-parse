...
class UnitextPlaintext(UnitextItem):
    ...
    def _remove_start(self, len0_ : int) -> 'UnitextPlaintext':
        ...
        if (self.id0_ is not None): 
            b.id0_ = "{0}_{1}_{2}".format(self.id0_, b.begin_char, b.end_char)
        else: 
            b.id0_ = "txtS_{0}_{1}".format(b.begin_char, b.end_char)
        if (self.layout is not None): 
    ...
    def _remove_end(self, len0_ : int) -> 'UnitextPlaintext':
        ...
        e0_.typ = self.typ
        if (self.id0_ is not None): 
            e0_.id0_ = "{0}_{1}_{2}".format(self.id0_, e0_.begin_char, e0_.end_char)
        else: 
            e0_.id0_ = "txtE_{0}_{1}".format(e0_.begin_char, e0_.end_char)
        if (self.layout is not None): 
        ...
        if ((par is not None and par.out_styles and self.get_styled_fragment(-1) is not None) and self.end_char >= self.begin_char): 
            ...
            if (has_inline): 
                ...
                if (cur is not None): 
                    ...
                    print("</span>", end="", file=res)
        if (not text_outed): 
            MiscHelper.correct_html_value(res, self.text, False, False, False)
        if (self.typ == UnitextPlaintextType.SUB): 
            print("</SUB>", end="", file=res)
        elif (self.typ == UnitextPlaintextType.SUP): 
            print("</SUP>", end="", file=res)
        ...
        if (xml0_.attrib is not None): 
            for a in xml0_.attrib.items(): 
                if (Utils.getXmlAttrLocalName(a) == "type"): 
                    ...
                    except Exception as ex579: 
                        pass
                elif (Utils.getXmlAttrLocalName(a) == "spaces"): 
                    wrapsp580 = RefOutArgWrapper(0)
                    Utils.tryParseInt(a[1], wrapsp580)
                    sp = wrapsp580.value
    ...
    @staticmethod
    def _new352(_arg1 : str, _arg2 : 'UnitextPlaintextType', _arg3 : str) -> 'UnitextPlaintext':
        res = UnitextPlaintext()
        res.text = _arg1
        res.typ = _arg2
    ...
    @staticmethod
    def _new379(_arg1 : 'UnitextItem') -> 'UnitextPlaintext':
        res = UnitextPlaintext()
        res.parent = _arg1
        return res
    
    @staticmethod
    def _new570(_arg1 : str, _arg2 : int, _arg3 : int) -> 'UnitextPlaintext':
        res = UnitextPlaintext()
        res.text = _arg1
