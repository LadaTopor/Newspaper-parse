        ...
        if (xml0_.attrib is not None): 
            for a in xml0_.attrib.items(): 
                if (Utils.getXmlAttrLocalName(a) == "number"): 
                    ...
                    i = 0
                    wrapi531 = RefOutArgWrapper(0)
                    inoutres532 = Utils.tryParseInt(a[1], wrapi531)
                    i = wrapi531.value
                    if (inoutres532): 
                        self.width = i
                elif (Utils.getXmlAttrLocalName(a) == "height"): 
                    i = 0
                    wrapi533 = RefOutArgWrapper(0)
                    inoutres534 = Utils.tryParseInt(a[1], wrapi533)
                    i = wrapi533.value
                    if (inoutres534): 
                        self.height = i
        for x in xml0_: 
            if (Utils.getXmlLocalName(x) == "rect"): 
                r = UnilayRectangle._new535(self)
                r.restore(x)
                self.rects.append(r)
