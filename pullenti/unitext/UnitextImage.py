...
class UnitextImage(UnitextItem):
    ...
    def get_html(self, res : io.StringIO, par : 'GetHtmlParam') -> None:
        ...
        if (self.width is not None and self.width.endswith("pt")): 
            nn = 0
            wrapnn567 = RefOutArgWrapper(0)
            inoutres568 = Utils.tryParseInt(self.width[0:0+len(self.width) - 2], wrapnn567)
            nn = wrapnn567.value
            if (inoutres568): 
                if (nn > 1000): 
                    print("width:100%;".format(), end="", file=res, flush=True)
        over = False
        ...
        if (self.content is not None): 
            if (over): 
                ...
                if (frm == FileFormat.JPG2000): 
                    ...
                    base640_ = base64.encodestring(self.content).decode('utf-8', 'ignore')
                    if (base640_.find('\n') >= 0): 
