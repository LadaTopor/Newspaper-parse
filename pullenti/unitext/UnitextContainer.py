...
class UnitextContainer(UnitextItem):
    ...
    def optimize(self, is_container : bool, pars : 'CreateDocumentParam') -> 'UnitextItem':
        ...
        for i in range(len(self.children) - 1, -1, -1):
            ...
            if (cnt.page_section_id is not None and self.page_section_id is not None and cnt.page_section_id != self.page_section_id): 
                continue
            if (cnt.typ == UnitextContainerType.SHAPE and len(cnt.children) > 0 and (isinstance(cnt.children[len(cnt.children) - 1], UnitextNewline))): 
                nl = cnt.children[len(cnt.children) - 1]
                del cnt.children[len(cnt.children) - 1]
                self.children.insert(i + 1, nl)
                nl.parent = (self)
                continue
        ...
        while True:
            ...
            if (i == 0 and i1 == (len(self.children) - 1) and self.typ == UnitextContainerType.UNDEFINED): 
                ...
                break
            cnt = UnitextContainer._new547(UnitextContainerType.MONOSPACE, self)
            j = i
            while j <= i1: 
    ...
    def get_html(self, res : io.StringIO, par : 'GetHtmlParam') -> None:
        ...
        if (self.html_title is not None): 
            tmp = io.StringIO()
            MiscHelper.correct_html_value(tmp, self.html_title, True, False, False)
    ...
    @staticmethod
    def _new376(_arg1 : int) -> 'UnitextContainer':
        res = UnitextContainer()
        res.end_char = _arg1
        return res
    
    @staticmethod
    def _new381(_arg1 : 'UnitextItem', _arg2 : int, _arg3 : int, _arg4 : str) -> 'UnitextContainer':
        res = UnitextContainer()
        res.parent = _arg1
    ...
    @staticmethod
    def _new431(_arg1 : 'UnitextContainerType', _arg2 : 'UnitextStyledFragment') -> 'UnitextContainer':
        res = UnitextContainer()
        res.typ = _arg1
    ...
    @staticmethod
    def _new547(_arg1 : 'UnitextContainerType', _arg2 : 'UnitextItem') -> 'UnitextContainer':
        res = UnitextContainer()
        res.typ = _arg1
        res.parent = _arg2
        return res
    ...
    @staticmethod
    def _new550(_arg1 : int) -> 'UnitextContainer':
        res = UnitextContainer()
