...
class UnitextFootnote(UnitextItem):
    ...
    def get_html(self, res : io.StringIO, par : 'GetHtmlParam') -> None:
        ...
        if (self.doc_block_id is not None): 
            ...
            MiscHelper.correct_html_value(res, Utils.ifNotNull(self.html_title, ""), True, False, False)
        ...
        if (par is not None and par.footnotes == GetHtmlParamFootnoteOutType.ENDOFUNIT): 
            ...
            print("<span title=\"".format(), end="", file=res, flush=True)
            MiscHelper.correct_html_value(res, Utils.toStringStringIO(tmp), True, False, False)
            if (self.is_endnote): 
                print("\" style=\"color:red\"><sup>&lt;E{0}&gt;</sup></span>".format(len(par._m_endnotes)), end="", file=res, flush=True)
            elif (self.custom_mark is not None): 
                print("\" style=\"color:red\"><sup>&lt;{0}&gt;</sup></span>".format(self.custom_mark), end="", file=res, flush=True)
        ...
        if (par is not None and par.footnotes == GetHtmlParamFootnoteOutType.INBRACKETS): 
            ...
            print("<span title=\"".format(), end="", file=res, flush=True)
            MiscHelper.correct_html_value(res, txt, True, False, False)
    ...
    @staticmethod
    def _new473(_arg1 : 'UnitextItem', _arg2 : str) -> 'UnitextFootnote':
        res = UnitextFootnote()
        res.content = _arg1
    ...
    @staticmethod
    def _new475(_arg1 : bool, _arg2 : 'UnitextItem') -> 'UnitextFootnote':
        res = UnitextFootnote()
    ...
    @staticmethod
    def _static_ctor():
        UnitextFootnote.__m_footnotes_param = GetPlaintextParam._new564(True, " ", " ", " ", " ", " ")
        UnitextFootnote.__m_footnotes_param1 = GetPlaintextParam._new564(False, " ", " ", " ", " ", " ")
