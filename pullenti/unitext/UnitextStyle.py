...
class UnitextStyle:
    ...
    def get_attr(self, name : str) -> str:
        ...
        """
        res = None
        wrapres581 = RefOutArgWrapper(None)
        inoutres582 = Utils.tryGetValue(self.attrs, name, wrapres581)
        res = wrapres581.value
        if (inoutres582): 
            return res
        return None
    ...
    def add_attr(self, name : str, val : str, append_val : bool=False) -> None:
        if (Utils.isNullOrEmpty(val)): 
            return
        val0 = None
        wrapval0583 = RefOutArgWrapper(None)
        inoutres584 = Utils.tryGetValue(self.attrs, name, wrapval0583)
        val0 = wrapval0583.value
        if (not inoutres584): 
            self.attrs[name] = val
