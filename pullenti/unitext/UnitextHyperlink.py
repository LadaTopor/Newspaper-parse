...
class UnitextHyperlink(UnitextItem):
    ...
    def __str__(self) -> str:
        return "Hyperlink {0} ({1})".format(Utils.ifNotNull(self.href, ""), Utils.ifNotNull(self.get_plaintext_string(None), ""))
    ...
    def get_html(self, res : io.StringIO, par : 'GetHtmlParam') -> None:
        ...
        if (self.html_title is not None or self.href is not None): 
            tmp = io.StringIO()
            MiscHelper.correct_html_value(tmp, Utils.ifNotNull(self.html_title, self.href), True, False, False)
        ...
        if (self.href is not None): 
            ...
            if (self.is_internal): 
                print('#', end="", file=res)
            MiscHelper.correct_html_value(res, self.href, True, False, False)
    ...
    @staticmethod
    def _new470(_arg1 : bool, _arg2 : str) -> 'UnitextHyperlink':
        res = UnitextHyperlink()
