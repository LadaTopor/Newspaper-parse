    ...
    def _out_footnotes(self, res : io.StringIO) -> None:
        ...
        if (len(self._m_footnotes) > 0): 
            ...
            while i < len(self._m_footnotes): 
                ...
                mark = Utils.ifNotNull(self._m_footnotes[i].custom_mark, "{0}".format(i + 1))
                if (sss.startswith("<div")): 
                    j = sss.find('>')
                    Utils.insertStringIO(tmp, j + 1, "<b id=\"{0}\"><sup>{1}</sup></b>".format(Utils.ifNotNull(self._m_footnotes[i].id0_, ""), mark))
                    print("\r\n", end="", file=res)
                    print(Utils.toStringStringIO(tmp), end="", file=res)
                else: 
                    print("\r\n<b id=\"{0}\"><sup>{1}</sup></b>".format(Utils.ifNotNull(self._m_footnotes[i].id0_, ""), mark), end="", file=res, flush=True)
