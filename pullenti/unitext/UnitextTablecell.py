...
class UnitextTablecell(UnitextItem):
    ...
    def get_html(self, res : io.StringIO, par : 'GetHtmlParam') -> None:
        if (not par.call_before(self, res)): 
            return
        print("<{0} valign=\"top\"".format(("th" if self.is_header else "td")), end="", file=res, flush=True)
        ...
        print(("</th>" if self.is_header else "</td>"), end="", file=res)
    ...
    def get_xml(self, xml0_ : XmlWriter) -> None:
        ...
        if (self.is_header): 
            xml0_.write_attribute_string("header", "true")
    ...
    @staticmethod
    def _new588(_arg1 : bool, _arg2 : int, _arg3 : int, _arg4 : int, _arg5 : int) -> 'UnitextTablecell':
        res = UnitextTablecell()
        res.is_psevdo = _arg1
        res.col_begin = _arg2
        res.col_end = _arg3
    ...
    @staticmethod
    def _new589(_arg1 : 'UnitextItem', _arg2 : int, _arg3 : int, _arg4 : int, _arg5 : int) -> 'UnitextTablecell':
        res = UnitextTablecell()
        res.content = _arg1
        res.col_begin = _arg2
        res.col_end = _arg3
