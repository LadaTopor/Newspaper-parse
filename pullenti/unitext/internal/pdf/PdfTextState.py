...
class PdfTextState:
    ...
    def __init__(self) -> None:
        ...
        self.font_size = 0
        self.font = None;
        self.__m_font_stack = list()
        self.__m_font_size_stack = list()
        self.char_space = 0
        self.word_space = 0
        self.wscale = 100
        self.leading = 0
    ...
    def parse_one(self, lex : typing.List['PdfObject'], i : int) -> bool:
        ...
        if (nam == "q"): 
            ...
            if (self.font is not None): 
                self.__m_font_stack.insert(0, self.font)
            self.__m_font_size_stack.insert(0, self.font_size)
            return True
        if (nam == "Q"): 
            if (len(self.ctm_stack) > 0): 
                del self.ctm_stack[0]
            if (len(self.__m_font_stack) > 0): 
                self.font = self.__m_font_stack[0]
                del self.__m_font_stack[0]
            if (len(self.__m_font_size_stack) > 0): 
                self.font_size = self.__m_font_size_stack[0]
                del self.__m_font_size_stack[0]
        return False
