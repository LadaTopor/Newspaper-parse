    ...
    def __copy_entry_direct(self, work_file : 'ZipFile', update : 'ZipUpdate', destination_position : int) -> None:
        ...
        if (skip_over): 
            ...
            if (update.entry.compressed_size > 0): 
                wrapsource_position522 = RefOutArgWrapper(source_position)
                self.__copy_entry_data_direct(update, self._m_base_stream, False, destination_position, wrapsource_position522)
                source_position = wrapsource_position522.value
            ...
            for up in self.__m_updates: 
                ...
                if (update is not None): 
                    ...
                    if (swichVal == UpdateCommand.COPY): 
                        if (direct_update): 
                            wrapdestination_position523 = RefOutArgWrapper(destination_position)
                            self.__copy_entry_direct(work_file, update, wrapdestination_position523)
                            destination_position = wrapdestination_position523.value
                        else: 
