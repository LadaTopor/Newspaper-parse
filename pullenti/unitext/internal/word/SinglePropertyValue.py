...
class SinglePropertyValue:
    ...
    def __parse_cmajority_operand(value : bytearray) -> object:
        with MemoryStream(value) as ms: 
            read = 0
            wrapread492 = RefOutArgWrapper(read)
            prls = BasicTypesReader._read_prls(ms, len(value), wrapread492)
            read = wrapread492.value
            result = dict()
            for prl in prls: 
    ...
    def __parsecnfoperand(value : bytearray) -> object:
        with MemoryStream(value) as ms: 
            read = 0
            result = dict()
            wrapread494 = RefOutArgWrapper(read)
            result["$FormattingCondition"] = int.from_bytes(ReadUtils._read_exact_ref(ms, ReadUtils._word_size, wrapread494)[0:0+2], byteorder="little")
            read = wrapread494.value
            wrapread493 = RefOutArgWrapper(read)
            prls = BasicTypesReader._read_prls(ms, len(value) - read, wrapread493)
            read = wrapread493.value
            for prl in prls: 
                name = SinglePropertyModifiers._map0_[prl._sprm._sprm]
                result[name[4:]] = SinglePropertyValue._parse_value(prl._sprm._sprm, prl._operand)
            return result
