...
class WordDocument:
                ...
                if (len(cps) > 2 and gen is not None): 
                    ...
                    while True:
                        ...
                        if (not cp in self.__m_footnotes): 
                            fn = UnitextFootnote._new475(k == 1, ge.finish(True, None))
    ...
    def __create_uni(self, gen : 'UnitextGen', blk : 'UnitextItem', i0 : int, p0 : int, p1 : int, txt : str, level : int) -> None:
        ...
        while True:
            ...
            if (p.is_list and gen is not None and not (isinstance(blk, UnitextList))): 
                ...
                if (data is not None and len(data) > 1): 
                    ...
                    if (ilfo >= 0 and (ilfo < len(self.__lfos))): 
                        wrapnum501 = RefOutArgWrapper(None)
                        Utils.tryGetValue(self.__m_num_styles, self.__lfos[ilfo].lsid, wrapnum501)
                        num = wrapnum501.value
            ...
            while True:
                ...
                sup = None
                is_sub = False
                wrapsup507 = RefOutArgWrapper(None)
                inoutres508 = Utils.tryGetValue(self.__m_super_texts, cp, wrapsup507)
                sup = wrapsup507.value
                if (inoutres508): 
                    pass
                else: 
                    wrapsup502 = RefOutArgWrapper(None)
                    inoutres503 = Utils.tryGetValue(self.__m_sub_texts, cp, wrapsup502)
                    sup = wrapsup502.value
                    if (inoutres503): 
                        is_sub = True
                if (sup is not None): 
                    if (cp == 84223): 
    ...
    def __load_paragraphs(self, word_document_stream : Stream, clx : 'Clx', plcf_bte_papx : 'PlcBtePapx') -> None:
        ...
        while i < len(plcf_bte_papx._apn_bte_papx): 
            ...
            er = False
            wraper510 = RefOutArgWrapper(False)
            papx_fkp = BasicTypesReader._read_papx_fkp(word_document_stream, wraper510)
            er = wraper510.value
            if (er): 
                self.__m_error = True
            j = 0
    ...
    def __expand_prm(self, prm : 'Prm', clx : 'Clx') -> typing.List['Prl']:
        if (prm._fcomplex): 
            ...
            sprm = 0
            wrapsprm511 = RefOutArgWrapper(0)
            inoutres512 = Utils.tryGetValue(SinglePropertyModifiers._prm0map, prm._isprm, wrapsprm511)
            sprm = wrapsprm511.value
            if (not inoutres512): 
    ...
    def __loadlstfs(self, word_document_stream : Stream, length : int) -> None:
        ...
        while i < cou: 
            ...
            while j < (((1 if lstf.fsimple_list else 9))): 
                ...
                mnem = None
                wrapmnem513 = RefOutArgWrapper(None)
                inoutres514 = Utils.tryGetValue(WordDocument.M_NUM_TYPES_MAP, lvl.nfc, wrapmnem513)
                mnem = wrapmnem513.value
                if (inoutres514): 
