...
class BasicTypesReader:
    ...
    def _read_clx(s : Stream, length : int) -> 'Clx':
        ...
        while clxt == Prc._default_clxt:
            read = 0
            wrapread391 = RefOutArgWrapper(0)
            prcs.append(BasicTypesReader.__read_prc_without_cltx(s, length - position, wrapread391))
            read = wrapread391.value
            position += read
            if (position >= length): 
                return None
            clxt = ReadUtils._read_byte(s)
    ...
    def __read_pcdt_without_clxt(s : Stream, length : int) -> 'Pcdt':
        ...
        wrapread392 = RefOutArgWrapper(read)
        pcdt._lcb = int.from_bytes(ReadUtils._read_exact_ref(s, ReadUtils._dword_size, wrapread392)[0:0+4], byteorder="little")
        read = wrapread392.value
    ...
    def __read_plc_pcd(s : Stream, length : int) -> 'PlcPcd':
        ...
        i = 0
        while i <= n: 
            wrapread393 = RefOutArgWrapper(read)
            plc_pcd._cps[i] = int.from_bytes(ReadUtils._read_exact_ref(s, ReadUtils._dword_size, wrapread393)[0:0+4], byteorder="little")
            read = wrapread393.value
            i += 1
        ...
        while position < length:
            ...
            if ((length - position) == 1): 
                ...
                try: 
                    ...
                    prl = BasicTypesReader.__read_prl(s, length - position, wrapread_portion394)
                    read_portion = wrapread_portion394.value
                    if (prl is None): 
                        break
                    prls.append(prl)
                except Exception as ex395: 
                    break
    ...
    def _read_papx_fkp(s : Stream, err : bool) -> 'PapxFkp':
        ...
        with MemoryStream(data) as data_stream: 
            ...
            while i < cpara: 
                ...
                try: 
                    wrapposition396 = RefOutArgWrapper(position)
                    grp_prl_and_istd._grpprl = BasicTypesReader._read_prls(data_stream, grpprl_in_papx_length - position, wrapposition396)
                    position = wrapposition396.value
    ...
    def _read_chpx_fkp(s : Stream) -> 'ChpxFkp':
        ...
        with MemoryStream(data) as data_stream: 
            ...
            while i < crun: 
                ...
                position = 0
                wrapposition397 = RefOutArgWrapper(position)
                chpx._grpprl = BasicTypesReader._read_prls(data_stream, grpprl_length - position, wrapposition397)
                position = wrapposition397.value
    ...
    def _read_stsh(s : Stream, stsh_length : int) -> 'STSH':
        ...
        stsh = STSH()
        read = 0
        wrapread401 = RefOutArgWrapper(read)
        cb_stshi = int.from_bytes(ReadUtils._read_exact_ref(s, ReadUtils._word_size, wrapread401)[0:0+2], byteorder="little")
        read = wrapread401.value
        read_portion = 0
        wrapread_portion400 = RefOutArgWrapper(0)
        stshi = BasicTypesReader.__read_stshi(s, cb_stshi, wrapread_portion400)
        read_portion = wrapread_portion400.value
        read += read_portion
        stsh._stshi = stshi
        stsh._rglpstd = Utils.newArray(stsh._stshi._stshif._cstd, None)
        i = 0
        while i < len(stsh._rglpstd): 
            wrapread399 = RefOutArgWrapper(read)
            cb_std = int.from_bytes(ReadUtils._read_exact_ref(s, ReadUtils._word_size, wrapread399)[0:0+2], byteorder="little")
            read = wrapread399.value
            if (cb_std > (0)): 
                wrapread_portion398 = RefOutArgWrapper(0)
                std = BasicTypesReader.__read_std(s, cb_std, stshi._stshif._cbstdbase_in_file, wrapread_portion398)
                read_portion = wrapread_portion398.value
                stsh._rglpstd[i] = std
                read += read_portion
            i += 1
        return stsh
    ...
    def __read_stk_paragrlpupx(s : Stream, length : int, read : int) -> 'StkParaGRLPUPX':
        ...
        stk_paragrlpupx._upx_papx = BasicTypesReader.__read_upx_papx(s, length - position, wrapposition406)
        position = wrapposition406.value
        wrapposition405 = RefOutArgWrapper(position)
        stk_paragrlpupx._upx_chpx = BasicTypesReader.__read_upx_chpx(s, length - position, wrapposition405)
        position = wrapposition405.value
        if (position < length): 
            stk_para_upx_grlpupxrm = StkParaUpxGrLPUpxRM()
            wrapposition404 = RefOutArgWrapper(position)
            stk_para_upx_grlpupxrm._upx_rm = BasicTypesReader.__read_upx_rm(s, length - position, wrapposition404)
            position = wrapposition404.value
            wrapposition403 = RefOutArgWrapper(position)
            stk_para_upx_grlpupxrm._upx_papxrm = BasicTypesReader.__read_upx_papx(s, length - position, wrapposition403)
            position = wrapposition403.value
            wrapposition402 = RefOutArgWrapper(position)
            stk_para_upx_grlpupxrm._upx_chpxrm = BasicTypesReader.__read_upx_chpx(s, length - position, wrapposition402)
    ...
    def __read_stk_chargrlpupx(s : Stream, length : int, read : int) -> 'StkCharGRLPUPX':
        ...
        stk_chargrlpupx._upx_chpx = BasicTypesReader.__read_upx_chpx(s, length - position, wrapposition409)
        position = wrapposition409.value
        if (position < length): 
            stk_char_upx_grlpupxrm = StkCharUpxGrLPUpxRM()
            wrapposition408 = RefOutArgWrapper(position)
            stk_char_upx_grlpupxrm._upx_rm = BasicTypesReader.__read_upx_rm(s, length - position, wrapposition408)
            position = wrapposition408.value
            wrapposition407 = RefOutArgWrapper(position)
            stk_char_upx_grlpupxrm._upx_chpxrm = BasicTypesReader.__read_upx_chpx(s, length - position, wrapposition407)
    ...
    def __read_stk_tablegrlpupx(s : Stream, length : int, read : int) -> 'StkTableGRLPUPX':
        ...
        position = 0
        wrapposition412 = RefOutArgWrapper(position)
        stk_tablegrlpupx._upx_tapx = BasicTypesReader.__read_upx_tapx(s, length - position, wrapposition412)
        position = wrapposition412.value
        wrapposition411 = RefOutArgWrapper(position)
        stk_tablegrlpupx._upx_papx = BasicTypesReader.__read_upx_papx(s, length - position, wrapposition411)
        position = wrapposition411.value
        wrapposition410 = RefOutArgWrapper(position)
        stk_tablegrlpupx._upx_chpx = BasicTypesReader.__read_upx_chpx(s, length - position, wrapposition410)
        position = wrapposition410.value
        read.value += position
    ...
    def __read_stk_listgrlpupx(s : Stream, length : int, read : int) -> 'StkListGRLPUPX':
        ...
        wrapposition413 = RefOutArgWrapper(position)
        stk_listgrlpupx._upx_papx = BasicTypesReader.__read_upx_papx(s, length - position, wrapposition413)
        position = wrapposition413.value
        read.value += position
