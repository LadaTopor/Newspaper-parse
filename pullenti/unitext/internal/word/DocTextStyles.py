...
class DocTextStyles:
    ...
    def register_style(self, st : 'UnitextStyle') -> 'UnitextStyle':
        ...
        if (Utils.isNullOrEmpty(key)): 
            return None
        res = None
        wrapres437 = RefOutArgWrapper(None)
        inoutres438 = Utils.tryGetValue(self.__m_ustyles, key, wrapres437)
        res = wrapres437.value
        if (inoutres438): 
            return res
    ...
    def get_style(self, x : xml.etree.ElementTree.Element, nam : str) -> 'DocTextStyle':
        ...
        if (id0_ is None): 
            return None
        res = None
        wrapres439 = RefOutArgWrapper(None)
        inoutres440 = Utils.tryGetValue(self.m_text_styles, id0_, wrapres439)
        res = wrapres439.value
        if (not inoutres440): 
            return None
        return res
    ...
    def __get_size_val(val : str) -> str:
        ...
        inoutres442 = MiscHelper.try_parse_double(val, wrapd441)
        d = wrapd441.value
        if (not inoutres442): 
            return val
        d = round(d / (20), 2)
    ...
    def read_unitext_style(self, node : xml.etree.ElementTree.Element, st : 'UnitextStyle') -> None:
        for x in node: 
            if (Utils.getXmlLocalName(x) == "pStyle" or Utils.getXmlLocalName(x) == "rStyle"): 
                ...
                d = 0
                wrapd443 = RefOutArgWrapper(0)
                inoutres444 = MiscHelper.try_parse_double(DocTextStyles.__read_attr_val(x, "val", None), wrapd443)
                d = wrapd443.value
                if (inoutres444): 
                ...
                for a in x.attrib.items(): 
                    if (Utils.getXmlAttrLocalName(a) == "before" or Utils.getXmlAttrLocalName(a) == "top"): 
                        ...
                        d = 0
                        wrapd445 = RefOutArgWrapper(0)
                        inoutres446 = MiscHelper.try_parse_double(a[1], wrapd445)
                        d = wrapd445.value
                        if (inoutres446): 
                            val = None
                            if (rule == "auto"): 
        ...
        for xnum in node: 
            if (Utils.getXmlLocalName(xnum) == "docDefaults"): 
                ...
                if (id0_ is None or id0_ in self.m_text_styles): 
                    continue
                sty = DocTextStyle()
                typ = DocTextStyles.__read_attr_val(xnum, "type", None)
                if (self.def_style is not None): 
                    self.def_style.copy_to(sty.ustyle)
                if (typ is not None and typ in self.def_styles): 
                    self.def_styles[typ].copy_to(sty.ustyle)
                        ...
                        if (x.attrib is not None): 
                            for a in x.attrib.items(): 
                                if (Utils.getXmlAttrLocalName(a) == "val" and a[1] in self.m_text_styles): 
                                    ...
                                    st0.ustyle.copy_to(sty.ustyle)
                    elif (Utils.getXmlLocalName(x) == "rPr"): 
                        self.read_unitext_style(x, sty.ustyle)
                        if (DocTextStyles.__read_attr_val(xnum, "default", None) == "1"): 
                            if (not typ in self.def_styles): 
                                self.def_styles[typ] = sty.ustyle
                                ...
                                for xxx in xx: 
                                    if (Utils.getXmlLocalName(xxx) == "numId" and xxx.attrib is not None): 
                                        ...
                                        for a in xxx.attrib.items(): 
                                            if (Utils.getXmlAttrLocalName(a) == "val"): 
                                                ll = 0
                                                wrapll447 = RefOutArgWrapper(0)
                                                inoutres448 = Utils.tryParseInt(a[1], wrapll447)
                                                ll = wrapll447.value
                                                if (inoutres448): 
                                                    sty.num_lvl = ll
                if (sty.is_heading): 
