...
class DocxToText(object):
    ...
    def __prepare_zip(self) -> None:
        for o in self.__zip_file.entries: 
            if (o.is_directory or o.encrypted or o.uncompress_data_size == 0): 
                continue
            self.__parts.append(DocxPart._new449(o.name, o))
    
    def __prepare_xml(self) -> None:
        if (Utils.getXmlLocalName(self.__xml_file.getroot()) == "wordDocument"): 
            self.__parts.append(DocxPart._new450("word/document.xml", self.__xml_file.getroot()))
            return
    ...
    def create_uni_doc(self, only_for_pure_text : bool, frm : 'FileFormat', pars : 'CreateDocumentParam') -> 'UnitextDocument':
            ...
            if (p.is_name("word/document.xml")): 
                                    ...
                                    if (xxx.attrib is not None): 
                                        for a in xxx.attrib.items(): 
                                            if (Utils.getXmlAttrLocalName(a) == "borderId"): 
                                                wrapind451 = RefOutArgWrapper(0)
                                                Utils.tryParseInt(a[1], wrapind451)
                                                ind = wrapind451.value
                                                break
                ...
                if (xml_rels is not None): 
                    ...
                    if (p.name.startswith("ppt")): 
                        ...
                        if (ii < 0): 
                            continue
                        wrapii452 = RefOutArgWrapper(0)
                        inoutres453 = Utils.tryParseInt(nam[0:0+ii], wrapii452)
                        ii = wrapii452.value
                        if (not inoutres453): 
                            continue
                        if (ii in ppt_images): 
                ...
                if (ii < 0): 
                    continue
                wrapii454 = RefOutArgWrapper(0)
                inoutres455 = Utils.tryParseInt(nam[0:0+ii], wrapii454)
                ii = wrapii454.value
                if (inoutres455): 
                    pass
                else: 
        ...
        if (xml_odt_content is not None): 
            ...
            return res
        if (len(ppt_slides) > 0): 
            res = UnitextDocument._new456(FileFormat.PPTX, self.__m_data_controls)
        ...
        if (xml_doc is not None): 
            res = UnitextDocument._new456(FileFormat.DOCX, self.__m_data_controls)
        ...
        if (xml_book is not None and self.__zip_file is not None): 
            res = UnitextDocument._new456(FileFormat.XLSX, self.__m_data_controls)
            books = dict()
            ...
            for o in self.__zip_file.entries: 
                ...
                if (id0_ is None): 
                    continue
                sheet_name = None
                wrapsheet_name459 = RefOutArgWrapper(None)
                Utils.tryGetValue(books, id0_, wrapsheet_name459)
                sheet_name = wrapsheet_name459.value
                dat = o.get_data()
                if (dat is None): 
                    continue
    ...
    def _read_node(self, stack_nodes : typing.List[xml.etree.ElementTree.Element], gen : 'UnitextGen', sfrag : 'UnitextStyledFragment', pict_top : int) -> None:
        ...
        while True:
            ...
            if (locname == "#text"): 
                continue
            if (Utils.getXmlLocalName(node) == "p"): 
                pass
            if (i == 7): 
                pass
            if (proof_err): 
            ...
            if (swichVal == "del"): 
                ...
                if (child.attrib is not None): 
                    for a in child.attrib.items(): 
                        if (Utils.getXmlAttrLocalName(a) == "id"): 
                            cmt = None
                            wrapcmt460 = RefOutArgWrapper(None)
                            inoutres461 = Utils.tryGetValue(self.__m_comments, Utils.ifNotNull(a[1], ""), wrapcmt460)
                            cmt = wrapcmt460.value
                            if (inoutres461): 
                                if (cmt.id0_ is None): 
                ...
                if (child.attrib is not None): 
                    for a in child.attrib.items(): 
                        if (Utils.getXmlAttrLocalName(a) == "id"): 
                            cmt = None
                            wrapcmt463 = RefOutArgWrapper(None)
                            inoutres464 = Utils.tryGetValue(self.__m_comments, Utils.ifNotNull(a[1], ""), wrapcmt463)
                            cmt = wrapcmt463.value
                            if (inoutres464): 
                                ecmt = UnitextComment._new462(cmt.id0_ + "_end", cmt.id0_, True)
                ...
                while True:
                    ...
                    if (sfrag is not None): 
                        para = UnitextStyledFragment._new468(sfrag, UnitextStyledFragmentType.PARAGRAPH)
                        sfrag.children.append(para)
                    def_para_style = None
                    if ("paragraph" in self.m_styles.def_styles): 
                        def_para_style = self.m_styles.register_style(self.m_styles.def_styles["paragraph"])
                        if (para is not None): 
                            para.style = def_para_style
                    pgen = UnitextGen()
                    pgen.set_style(para)
                    ...
                    for xx in child: 
                        if (Utils.getXmlLocalName(xx) == "pPr"): 
                            if (para is not None): 
                                ust0 = UnitextStyle()
                                if (def_para_style is not None): 
                                    def_para_style.copy_to(ust0)
                                ...
                                if (Utils.getXmlLocalName(xxx) == "numPr"): 
                                    for chh in xxx: 
                                        if (Utils.getXmlLocalName(chh) == "numId" and chh.attrib is not None and len(chh.attrib) == 1): 
                                            ...
                                            wraplev469 = RefOutArgWrapper(0)
                                            Utils.tryParseInt(Utils.ifNotNull(Utils.getXmlAttrByIndex(chh.attrib, 0)[1], ""), wraplev469)
                                            lev = wraplev469.value
                ...
                while True:
                    ...
                    if (fld_char_begin): 
                        ...
                        while True:
                            ...
                            if (not (j < len(node))): break
                            chh = node[j]
                            if (Utils.getXmlLocalName(chh) == "bookmarkStart"): 
                                for a in chh.attrib.items(): 
                                    if (Utils.getXmlAttrLocalName(a) == "name"): 
                                        if (a[1] == "_GoBack"): 
                                            continue
                                        mi = UnitextMisc()
                        ...
                        if (anchor is not None): 
                            hy = UnitextHyperlink._new470(True, anchor)
                    ...
                    if (ust is None or sfrag is None): 
                        ...
                        ifr = UnitextStyledFragment._new468(sfrag, UnitextStyledFragmentType.INLINE)
                        ifr.style = ust
                        gg1 = UnitextGen()
                        gg1.set_style(ifr)
                        gg1.append_pagesection(self.__m_cur_section)
                ...
                while True:
                    ...
                    for a in child.attrib.items(): 
                        if (Utils.getXmlAttrLocalName(a) == "id" and a[1] in self.__m_footnotes): 
                            ...
                            gen.append(UnitextFootnote._new473(cnt, del_text), None, -1, False)
                            break
                    break
                ...
                while True:
                    ...
                    if (tmp2 is None): 
                        break
                    fn = UnitextFootnote._new475(locname == "endnote", tmp2)
                                    ...
                                    if (Utils.getXmlLocalName(xxx) == "extent" and xxx.attrib is not None): 
                                        ...
                                        for aa in xxx.attrib.items(): 
                                            if (Utils.getXmlAttrLocalName(aa) == "cx"): 
                                                wrapxw477 = RefOutArgWrapper(0)
                                                Utils.tryParseInt(aa[1], wrapxw477)
                                                xw = wrapxw477.value
                                            elif (Utils.getXmlAttrLocalName(aa) == "cy"): 
                                                wrapyh478 = RefOutArgWrapper(0)
                                                Utils.tryParseInt(aa[1], wrapyh478)
                                                yh = wrapyh478.value
                                        if (xw > 0 and yh > 0): 
                ...
                for x in child: 
                    if (Utils.getXmlLocalName(x) == "sdtContent"): 
                        sfrag1 = sfrag
                        if (ust1 is not None): 
                            sfrag1 = UnitextStyledFragment._new480(UnitextStyledFragmentType.INLINE, ust1, sfrag1)
                        ...
                        if (isinstance(tmp3, UnitextContainer)): 
                            ...
                            ccc = UnitextContainer._new431(UnitextContainerType.CONTENTCONTROL, sfrag1)
                    ...
                    if (Utils.getXmlAttrLocalName(a) == "style"): 
                        ...
                        for s in Utils.splitString(a[1], ';', False): 
                            ...
                            for j in range(len(va) - 1, 0, -1):
                                if (str.isdigit(va[j])): 
                                    wrapii482 = RefOutArgWrapper(0)
                                    inoutres483 = Utils.tryParseInt(va[0:0+j + 1], wrapii482)
                                    ii = wrapii482.value
                                    if (not inoutres483): 
                                        break
                                    if (ii > pict_top): 
                ...
                del stack_nodes[len(stack_nodes) - 1]
                drsh = UnitextContainer._new431(UnitextContainerType.SHAPE, sfrag)
