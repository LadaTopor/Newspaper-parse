...
class FATStream(CompoundFileStream):
    ...
    def __init__(self, storage : 'CompoundFileStorage') -> None:
        super().__init__(storage, storage._system._get_sector_size())
    
    def _get_page_data(self, page_index : int) -> bytearray:
        page = Utils.newArrayOfBytes(self._page_size, 0)
        sector = self._system._get_stream_next_sector(self._storage._entry._starting_sector_location, page_index)
        return ReaderUtils._read_fragment(self._system.base_stream, self._system._get_sector_offset(sector), self._page_size)