        ...
        for xnum in node: 
            if (Utils.getXmlLocalName(xnum) == "abstractNum" and xnum.attrib is not None and len(xnum.attrib) >= 1): 
                        ...
                        for xxx in xx: 
                            if (xxx.attrib is not None): 
                                if (Utils.getXmlLocalName(xxx) == "numFmt"): 
                                    ...
                                    ii = 0
                                    wrapii415 = RefOutArgWrapper(0)
                                    inoutres416 = Utils.tryParseInt(Utils.ifNotNull(DocNumStyles.__read_attr_val(xxx, "val", None), ""), wrapii415)
                                    ii = wrapii415.value
                                    if (inoutres416): 
                                        nlev.start = ii
                ...
                if (int_id is None or id0_ is None): 
                    continue
                num0 = None
                wrapnum0422 = RefOutArgWrapper(None)
                inoutres423 = Utils.tryGetValue(abstr, int_id, wrapnum0422)
                num0 = wrapnum0422.value
                if (not inoutres423): 
                    continue
                num = UnitextGenNumStyleEx._new417(num0)
                num.id0_ = id0_
                ...
                for xx in xnum: 
                    if (Utils.getXmlLocalName(xx) == "lvlOverride"): 
                        l_ = 0
                        wrapl420 = RefOutArgWrapper(0)
                        inoutres421 = Utils.tryParseInt(Utils.ifNotNull(DocNumStyles.__read_attr_val(xx, "val", None), ""), wrapl420)
                        l_ = wrapl420.value
                        if (not inoutres421): 
                            continue
                        ...
                        for xxx in xx: 
                            if (Utils.getXmlLocalName(xxx) == "startOverride"): 
                                s = 0
                                wraps418 = RefOutArgWrapper(0)
                                inoutres419 = Utils.tryParseInt(Utils.ifNotNull(DocNumStyles.__read_attr_val(xxx, "val", None), ""), wraps418)
                                s = wraps418.value
                                if (inoutres419): 
    ...
    def _read_number_style(xml0_ : xml.etree.ElementTree.Element) -> 'UnitextGenNumStyle':
        ...
        for x in xml0_: 
            if (Utils.getXmlLocalName(x) == "ilvl"): 
                ...
                nn = 0
                wrapnn424 = RefOutArgWrapper(0)
                inoutres425 = Utils.tryParseInt(Utils.ifNotNull(DocNumStyles.__read_attr_val(x, "val", None), ""), wrapnn424)
                nn = wrapnn424.value
                if (inoutres425): 
                    res.lvl = nn
