...
class SinglePropertyModifiers:
    ...
    def _get_sprm_by_name(name : str) -> int:
        res = 0
        wrapres488 = RefOutArgWrapper(0)
        inoutres489 = Utils.tryGetValue(SinglePropertyModifiers._map2, name, wrapres488)
        res = wrapres488.value
        if (inoutres489): 
            return res
    ...
    def get_sprm_name(sprm : int) -> str:
        res = None
        wrapres490 = RefOutArgWrapper(None)
        inoutres491 = Utils.tryGetValue(SinglePropertyModifiers._map0_, sprm, wrapres490)
        res = wrapres490.value
        if (not inoutres491): 
            return None
        else: 
            return res
