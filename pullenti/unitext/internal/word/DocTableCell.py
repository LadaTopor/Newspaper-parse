...
class DocTableCell:
    ...
    def read(self, own : 'DocxToText', sfrag : 'UnitextStyledFragment', stack_nodes : typing.List[xml.etree.ElementTree.Element]) -> None:
        ...
        if (sfrag is not None): 
            self.frag = UnitextStyledFragment._new428(UnitextStyledFragmentType.TABLECELL, sfrag)
            ...
            if (Utils.getXmlLocalName(x) == "tcPr"): 
                ...
                for xx in x: 
                    if (Utils.getXmlLocalName(xx) == "gridSpan" and xx.attrib is not None and len(xx.attrib) == 1): 
                        cou = 0
                        wrapcou435 = RefOutArgWrapper(0)
                        inoutres436 = Utils.tryParseInt(Utils.getXmlAttrByIndex(xx.attrib, 0)[1], wrapcou435)
                        cou = wrapcou435.value
                        if (inoutres436): 
                            if (cou > 1): 
                                self.col_span = cou
