...
class UnitextHelper:
    ...
    def create(file_name : str, file_content : bytearray, pars : 'CreateDocumentParam') -> 'UnitextDocument':
        ...
        if (file_content is None): 
            if (file_name is None): 
                return UnitextDocument._new361(FileFormat.UNKNOWN, "Not filename, not content...".format(file_name))
            file_name = pathlib.Path(file_name).resolve()
            if (not pathlib.Path(file_name).is_file()): 
                return UnitextDocument._new361(FileFormat.UNKNOWN, "File '{0}' not exists".format(file_name))
        ...
        try: 
            ...
            if (doc is None): 
                doc = UnitextDocument._new361(frm, ("Format unknown" if frm == FileFormat.UNKNOWN else "Can't create document"))
        except Exception as ex: 
            doc = UnitextDocument._new361(frm, str(ex))
        ...
        for h in in_hyps: 
            anc = None
            wrapanc366 = RefOutArgWrapper(None)
            inoutres367 = Utils.tryGetValue(ancs, h.href, wrapanc366)
            anc = wrapanc366.value
            if (not inoutres367): 
                continue
                ...
                if (i >= 0): 
                    for j in range(i - 1, -1, -1):
                        ...
                        if (((isinstance(it, UnitextNewline)) or (isinstance(it, UnitextPagebreak)) or (isinstance(it, UnitextTable))) or (isinstance(it, UnitextList))): 
                            break
                        if (isinstance(it, UnitextComment)): 
                            continue
                    ...
                    if (ait is None): 
                        ...
                        while True:
                            ...
                            it = cnt.children[j]
                            if (((isinstance(it, UnitextNewline)) or (isinstance(it, UnitextMisc)) or (isinstance(it, UnitextPagebreak))) or (isinstance(it, UnitextComment))): 
                                continue
                            ait = it
                            break
                    if (ait is not None): 
    ...
    def __create(frm : 'FileFormat', file_name : str, file_content : bytearray, pars : 'CreateDocumentParam') -> 'UnitextDocument':
        ...
        if (frm == FileFormat.HTML): 
            ...
            if (not pars.only_for_pure_text and file_name is not None and pathlib.Path(file_name).is_file()): 
                dir0_ = pathlib.PurePath(pathlib.Path(file_name).resolve()).parent.absolute()
        ...
        if (frm == FileFormat.MHT or frm == FileFormat.EML): 
            ...
            try: 
                ...
                if (doc is not None): 
                    ...
                    doc = UnitextDocument._new361(frm, "Не удалось сформировать письмо")
                return doc
            finally: 
                if (fs is not None): 
                    fs.close()
    ...
    def create_doc_from_text(text : str) -> 'UnitextDocument':
        from pullenti.unitext.UnitextNewline import UnitextNewline
        from pullenti.unitext.UnitextDocument import UnitextDocument
        doc = UnitextDocument._new375(Utils.ifNotNull(text, ""))
        if (Utils.isNullOrEmpty(text)): 
            return doc
        cur = None
        cnt = UnitextContainer._new376(len(text) - 1)
        doc.content = (cnt)
        cnt.parent = (doc)
        ...
        while i < len(text): 
            ...
            if (ch == '\r' or ch == '\n'): 
                if (not (isinstance(cur, UnitextNewline))): 
                    cur = (UnitextNewline._new377(cnt))
                ...
                cur = (UnitextPagebreak._new378(cnt))
                cnt.children.append(cur)
                cur.begin_char = i
                cur.end_char = i
            else: 
                if (not (isinstance(cur, UnitextPlaintext))): 
                    cur = (UnitextPlaintext._new379(cnt))
                    cnt.children.append(cur)
                    cur.begin_char = i
                cur.end_char = i
            i += 1
