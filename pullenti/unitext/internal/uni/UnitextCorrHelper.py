...
from pullenti.unitext.internal.uni.UnitextCorrPdfPage import UnitextCorrPdfPage
...
class UnitextCorrHelper:
    ...
    def restore_text_footnotes(doc : 'UnitextDocument') -> None:
        ...
        if (cnt is None): 
            return
        if (doc.source_format == FileFormat.PDF or doc.source_format == FileFormat.DJVU): 
            UnitextCorrPdfPage.correct(cnt)
            return
        fcl = list()
        num_start = 1
        i = 0
