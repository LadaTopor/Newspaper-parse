...
class UnitextImplantateHelper:
    ...
    @staticmethod
    def __split_texts(content : 'UnitextItem', begin : int, end : int, own : 'UnitextContainer', lev : int) -> 'UnitextItem':
        if (content is None): 
            return None
        if (lev > 30): 
            return content
        ...
        if (tab is not None): 
            ...
            if (cel is not None and cel.content is not None): 
                cel.content = UnitextImplantateHelper.__split_texts(cel.content, begin, end, None, lev + 1)
            return tab
        li = Utils.asObjectOrNull(content, UnitextList)
        if (li is not None): 
            for l_ in li.items: 
                if (l_.prefix is not None): 
                    l_.prefix = UnitextImplantateHelper.__split_texts(l_.prefix, begin, end, None, lev + 1)
                if (l_.content is not None): 
                    l_.content = UnitextImplantateHelper.__split_texts(l_.content, begin, end, None, lev + 1)
                if (l_.sublist is not None): 
                    UnitextImplantateHelper.__split_texts(l_.sublist, begin, end, None, lev + 1)
            return li
        foot = Utils.asObjectOrNull(content, UnitextFootnote)
        if (foot is not None): 
            if (foot.content is not None): 
                foot.content = UnitextImplantateHelper.__split_texts(foot.content, begin, end, None, lev + 1)
            return foot
        hyp = Utils.asObjectOrNull(content, UnitextHyperlink)
        if (hyp is not None): 
            if (hyp.content is not None): 
                hyp.content = UnitextImplantateHelper.__split_texts(hyp.content, begin, end, None, lev + 1)
            return hyp
        dbl = Utils.asObjectOrNull(content, UnitextDocblock)
        if (dbl is not None): 
            if (dbl.head is not None): 
                hh = Utils.asObjectOrNull(UnitextImplantateHelper.__split_texts(dbl.head, begin, end, None, lev + 1), UnitextContainer)
                if (dbl in hh.children): 
                    pass
                dbl.head = hh
            if (dbl.body is not None): 
                dbl.body = UnitextImplantateHelper.__split_texts(dbl.body, begin, end, None, lev + 1)
            if (dbl.tail is not None): 
                dbl.tail = (Utils.asObjectOrNull(UnitextImplantateHelper.__split_texts(dbl.tail, begin, end, None, lev + 1), UnitextContainer))
            if (dbl.appendix is not None): 
                dbl.appendix = UnitextImplantateHelper.__split_texts(dbl.appendix, begin, end, None, lev + 1)
            return dbl
        cnt = Utils.asObjectOrNull(content, UnitextContainer)
        if (cnt is not None): 
            i = 0
            ...
            while True:
                ...
                if (ch.begin_char > end): 
                    if (end == (begin - 1) and ch.begin_char <= begin): 
                        ...
                        break
                cc = UnitextImplantateHelper.__split_texts(ch, begin, end, cnt, lev + 1)
                if (cc is None): 
                    pass
                cnt.children[i] = cc
            return content
        ...
        if (txt is not None): 
            ...
            creat = False
            if (own is None): 
                own = UnitextContainer._new381(txt.parent, txt.begin_char, txt.end_char, "cnt0_{0}_{1}".format(txt.begin_char, txt.end_char))
    ...
    def implantate(content : 'UnitextItem', impl : 'UnitextContainer', text : str, own : 'UnitextContainer'=None) -> 'UnitextItem':
        ...
        if ((impl.begin_char == content.begin_char and impl.end_char == content.end_char and own is None) and not (isinstance(content, UnitextContainer))): 
            ...
            content.parent = (impl)
            return impl
        content = UnitextImplantateHelper.__split_texts(content, impl.begin_char, impl.end_char, own, 0)
        if (content is None): 
            return None
        if (content == impl): 
            return content
        ...
        if (li is not None): 
            for l_ in li.items: 
                if (l_.begin_char <= impl.begin_char and impl.begin_char <= l_.end_char): 
                    if (l_.prefix is not None and impl.parent is None): 
                        l_.prefix = UnitextImplantateHelper.implantate(l_.prefix, impl, text, None)
                    if (l_.content is not None and impl.parent is None): 
                        l_.content = UnitextImplantateHelper.implantate(l_.content, impl, text, None)
                    if (l_.sublist is not None and impl.parent is None): 
        ...
        if (cnt is not None): 
            ...
            while True:
                ...
                if (j > i): 
                    ...
                    impl.begin_char = impl.children[0].begin_char
                break
            return content
        impl.children.append(content)
        impl.parent = content.parent
    ...
    def implantate_block(content : 'UnitextItem', begin_head : int, begin_body : int, begin_tail : int, begin_appendix : int, end : int, text : str, own : 'UnitextContainer', res : 'UnitextDocblock') -> 'UnitextItem':
        ...
        if ((begin_head < begin_body) and (begin_body < end)): 
            content = UnitextImplantateHelper.__split_texts(content, begin_head, begin_body - 1, own, 0)
        tab = Utils.asObjectOrNull(content, UnitextTable)
        if (tab is not None): 
            bh = begin_head
            while bh < len(text): 
                ...
                while True:
                    ...
                    if (cel.begin_char <= bh and bh <= cel.end_char and cel.row_begin == cel.row_end): 
                        if ((bh < bb) and cel.end_char > bh and bb > cel.end_char): 
                            res0 = UnitextDocblock._new382(bh, cel.end_char)
        ...
        if (cnt is not None): 
            ...
            while True:
                ...
                if ((isinstance(ch, UnitextTable)) and ch.begin_char <= begin_head and (end < ch.end_char)): 
                    UnitextImplantateHelper.implantate_block(ch, begin_head, begin_body, begin_tail, begin_appendix, end, text, None, res)
                    return content
                res.value = UnitextDocblock._new382(begin_head, end)
                ...
                if (j > i): 
                    ...
                    if (len(body.children) > 0): 
                        ...
                        if (body.children[0].begin_char < body.begin_char): 
                            body.begin_char = body.children[0].begin_char
                    ...
                    if (len(tail.children) > 0): 
                        ...
                        if (tail.children[0].begin_char < tail.begin_char): 
                            tail.begin_char = tail.children[0].begin_char
