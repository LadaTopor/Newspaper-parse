...
class UnitextTable(UnitextItem):
        ...
        if (self.__m_cols_count == 1): 
            ...
            while r < len(self.__m_cells): 
                if (self.__m_cells[r][0].content is not None and self.__m_cells[r][0].row_begin == r): 
                    ...
                    cnt.children.append(UnitextNewline._new586(cnt))
                r += 1
            cnt.children.insert(0, UnitextNewline._new586(cnt))
        ...
        if (pars is not None and pars.split_table_rows): 
            ...
            while True:
                ...
                while True:
                    ...
                    if (not (c < self.cols_count)): break
                    cel = self.get_cell(r, c)
                    cel1 = UnitextTablecell._new588(True, c, c, r + 1, r + 1)
                    nrow.append(cel1)
                    if (cel.content is None): 
                        continue
                    cnt = Utils.asObjectOrNull(cel.content, UnitextContainer)
        ...
        for r in range(row_end, row_begin - 1, -1):
            ...
            while c <= col_end: 
                if (self.__m_cells[r][c] is not None): 
                    ...
                    return None
                c += 1
        cel = UnitextTablecell._new589(content, col_begin, col_end, row_begin, row_end)
        r = row_begin
        while r <= row_end: 
            c = col_begin
            while c <= col_end: 
    ...
    def append_table(self, tab : 'UnitextTable', dont_clone : bool, check_width : bool) -> bool:
        ...
        if (check_width): 
            ...
            while True:
                ...
                n1 = 0
                n2 = 0
                wrapn1592 = RefOutArgWrapper(0)
                inoutres593 = Utils.tryParseInt(w1, wrapn1592)
                wrapn2594 = RefOutArgWrapper(0)
                inoutres595 = Utils.tryParseInt(w2, wrapn2594)
                n1 = wrapn1592.value
                n2 = wrapn2594.value
                if (not inoutres593 or not inoutres595): 
                    return False
                d = n1 - n2
                if (d < 0): 
                    d = (- d)
                ...
                if (((k0 + 1) < self.cols_count) and (merge_col < 0)): 
                    ...
                    if (w3 is not None and w3.endswith("%")): 
                        w3 = w3[0:0+len(w3) - 1]
                    n3 = 0
                    wrapn3590 = RefOutArgWrapper(0)
                    inoutres591 = Utils.tryParseInt(Utils.ifNotNull(w3, ""), wrapn3590)
                    n3 = wrapn3590.value
                    if (inoutres591): 
                        d = (n2 - n3)
                        if (d < 0): 
    ...
    def _try_append(self, tab : 'UnitextTable') -> bool:
        ...
        while True:
            ...
            if (w1 == w2): 
                continue
            su1 = SizeunitConverter(w1)
            su2 = SizeunitConverter(w2)
            if (su1.unit != su2.unit): 
                return False
            d = math.fabs(su1.val - su2.val)
            if (d > 1): 
                return False
        for k in range(2):
        ...
        for r in range(tab.rows_count - 1, -1, -1):
            ...
            first_pass734 = True
            while True:
                if first_pass734: first_pass734 = False
                else: c += 1
    ...
    def get_plaintext(self, res : io.StringIO, pars : 'GetPlaintextParam'=None) -> None:
        ...
        while r < len(self.__m_cells): 
            ...
            first_pass735 = True
            while True:
                if first_pass735: first_pass735 = False
                else: c += 1
                if (not (c < len(self.__m_cells[r]))): break
                cel = self.__m_cells[r][c]
                if (cel is None): 
    ...
    def get_html(self, res : io.StringIO, par : 'GetHtmlParam') -> None:
        ...
        while r < len(self.__m_cells): 
            ...
            first_pass736 = True
            while True:
                if first_pass736: first_pass736 = False
                else: c += 1
                if (not (c < len(self.__m_cells[r]))): break
                cel = self.__m_cells[r][c]
                if (cel is None): 
    ...
    def get_xml(self, xml0_ : XmlWriter) -> None:
        ...
        while r < len(self.__m_cells): 
            ...
            first_pass737 = True
            while True:
                if first_pass737: first_pass737 = False
                else: c += 1
                if (not (c < len(self.__m_cells[r]))): break
                cel = self.__m_cells[r][c]
                if (cel is None): 
                ...
                if (x.attrib is not None): 
                    for a in x.attrib.items(): 
                        if (Utils.getXmlAttrLocalName(a) == "width"): 
                            ...
                            wrapnum596 = RefOutArgWrapper(0)
                            Utils.tryParseInt(Utils.ifNotNull(a[1], ""), wrapnum596)
                            num = wrapnum596.value
                if (val is not None): 
    ...
    def get_all_items(self, res : typing.List['UnitextItem'], lev : int) -> None:
        ...
        while r < len(self.__m_cells): 
            ...
            first_pass738 = True
            while True:
                if first_pass738: first_pass738 = False
                else: c += 1
                if (not (c < len(self.__m_cells[r]))): break
                cel = self.__m_cells[r][c]
                if (cel is None): 
    ...
    def _add_plain_text_pos(self, d : int) -> None:
        ...
        while r < len(self.__m_cells): 
            ...
            first_pass739 = True
            while True:
                if first_pass739: first_pass739 = False
                else: c += 1
                if (not (c < len(self.__m_cells[r]))): break
                cel = self.__m_cells[r][c]
                if (cel is None): 
    ...
    def _correct(self, typ : 'LocCorrTyp', data : object) -> None:
        ...
        while r < len(self.__m_cells): 
            ...
            first_pass740 = True
            while True:
                if first_pass740: first_pass740 = False
                else: c += 1
                if (not (c < len(self.__m_cells[r]))): break
                cel = self.__m_cells[r][c]
                if (cel is None): 
    ...
    def _set_default_text_pos(self, cp : int, res : io.StringIO) -> None:
        ...
        while r < len(self.__m_cells): 
            ...
            first_pass741 = True
            while True:
                if first_pass741: first_pass741 = False
                else: c += 1
                if (not (c < len(self.__m_cells[r]))): break
                cel = self.__m_cells[r][c]
                if (cel is None): 
