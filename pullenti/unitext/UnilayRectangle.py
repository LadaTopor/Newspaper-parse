...
class UnilayRectangle:
        ...
        if (xml0_.attrib is not None): 
            for a in xml0_.attrib.items(): 
                if (Utils.getXmlAttrLocalName(a) == "line"): 
                    i = 0
                    wrapi536 = RefOutArgWrapper(0)
                    inoutres537 = Utils.tryParseInt(a[1], wrapi536)
                    i = wrapi536.value
                    if (inoutres537): 
                        self.line_number = i
                    ...
                    wrapd542 = RefOutArgWrapper(0)
                    inoutres543 = MiscHelper.try_parse_double(a[1], wrapd542)
                    d = wrapd542.value
                    if (inoutres543): 
                        self.right = d
                elif (Utils.getXmlAttrLocalName(a) == "bottom"): 
                    wrapd544 = RefOutArgWrapper(0)
                    inoutres545 = MiscHelper.try_parse_double(a[1], wrapd544)
                    d = wrapd544.value
                    if (inoutres545): 
                        self.bottom = d
    ...
    @staticmethod
    def _new535(_arg1 : 'UnilayPage') -> 'UnilayRectangle':
        res = UnilayRectangle()
        res.page = _arg1
        return res