...
class UnitextDocblock(UnitextItem):
    ...
    def get_html(self, res : io.StringIO, par : 'GetHtmlParam') -> None:
        ...
        if (self.html_title is not None): 
            ...
            MiscHelper.correct_html_value(res, self.html_title, True, False, False)
            print("\"", end="", file=res)
        margleft = 0
    ...
    def _append_child(self, it : 'UnitextItem') -> bool:
        if (self.appendix is not None): 
            ...
            if (cnt is None): 
                cnt = UnitextContainer._new550(self.body.begin_char)
    ...
    @staticmethod
    def _new382(_arg1 : int, _arg2 : int) -> 'UnitextDocblock':
        res = UnitextDocblock()
