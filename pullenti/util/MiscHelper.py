...
class MiscHelper:
    ...
    def try_parse_float(str0_ : str, res : float) -> bool:
        ...
        if (Utils.isNullOrEmpty(str0_)): 
            return False
        inoutres605 = Utils.tryParseFloat(str0_, res)
        if (inoutres605): 
            return True
        inoutres604 = Utils.tryParseFloat(str0_.replace(',', '.'), res)
        if (str0_.find(',') >= 0 and inoutres604): 
            return True
        inoutres603 = Utils.tryParseFloat(str0_.replace('.', ','), res)
        if (str0_.find('.') >= 0 and inoutres603): 
            return True
        return False
    ...
    def try_parse_double(str0_ : str, res : float) -> bool:
        ...
        if (Utils.isNullOrEmpty(str0_)): 
            return False
        inoutres608 = Utils.tryParseFloat(str0_, res)
        if (inoutres608): 
            return True
        inoutres607 = Utils.tryParseFloat(str0_.replace(',', '.'), res)
        if (str0_.find(',') >= 0 and inoutres607): 
            return True
        inoutres606 = Utils.tryParseFloat(str0_.replace('.', ','), res)
        if (str0_.find('.') >= 0 and inoutres606): 
            return True
        return False
    ...
    def encode_string1251(str0_ : str) -> bytearray:
        ...
        while j < len(str0_): 
            ...
            if (i < 0x80): 
                ...
                b = 0
                wrapb609 = RefOutArgWrapper(0)
                inoutres610 = Utils.tryGetValue(MiscHelper.__m_utf_1251, i, wrapb609)
                b = wrapb609.value
                if (inoutres610): 
                    res[j] = b
                else: 
    ...
    def encode_string1252(str0_ : str) -> bytearray:
        ...
        while j < len(str0_): 
            ...
            if ((i < 0x80) or i >= 0xA0): 
                ...
                b = 0
                wrapb611 = RefOutArgWrapper(0)
                inoutres612 = Utils.tryGetValue(MiscHelper.__m_utf_1252, i, wrapb611)
                b = wrapb611.value
                if (inoutres612): 
                    res[j] = b
                else: 
    ...
    def correct_xml_file(res : io.StringIO) -> None:
        ...
        first_pass743 = True
        while True:
            if first_pass743: first_pass743 = False
            else: i += 1
    ...
    @staticmethod
    def correct_html_value(res : io.StringIO, txt : str, is_attr : bool=False, is_pure_xml : bool=False, encode_all_nonansi_chars : bool=False) -> None:
        if (txt is None): 
            return
        i = 0
        while i < len(txt): 
            ...
            if ((((ord(c)) < 0x20) and c != '\r' and c != '\n') and c != '\t'): 
                print(' ', end="", file=res)
            elif ((ord(c)) > 0xF000 or ((encode_all_nonansi_chars and (ord(c)) >= 0x80))): 
                print("&#{0};".format(ord(c)), end="", file=res, flush=True)
            elif ((ord(c)) == 0xA or (ord(c)) == 0xD): 
