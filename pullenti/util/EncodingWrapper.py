...
class EncodingWrapper:
    ...
    def __init__(self, typ : 'EncodingStandard', str0_ : str=None, cp : int=0) -> None:
        ...
        if (ss.startswith("WINDOWS")): 
            cod = 0
            wrapcod598 = RefOutArgWrapper(0)
            inoutres599 = Utils.tryParseInt(ss[7:], wrapcod598)
            cod = wrapcod598.value
            if (inoutres599): 
                self.code_page = cod
                if (cod == 1251): 
                    self.std_typ = EncodingStandard.CP1251
        ...
        if (ss.startswith("CP")): 
            cod = 0
            wrapcod600 = RefOutArgWrapper(0)
            inoutres601 = Utils.tryParseInt(ss[2:], wrapcod600)
            cod = wrapcod600.value
            if (inoutres601): 
                self.code_page = cod
                if (cod == 1251): 
                    self.std_typ = EncodingStandard.CP1251
    ...
    def check_encoding(data : bytearray, head_len : int) -> 'EncodingWrapper':
        ...
        first_pass742 = True
        while True:
            if first_pass742: first_pass742 = False
            else: i += 1
